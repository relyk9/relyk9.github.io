...
const Home: React.FC = () => {
  ...
  return (
    <div className="space-y-8 md:space-y-16 animate-in fade-in duration-1000">
      <section className="bg-black/80 border border-emerald-500 p-6 md:p-12 rounded shadow-[0_0_40px_rgba(16,185,129,0.05)] relative overflow-hidden">
        
        <div className="grid grid-cols-1 md:grid-cols-[1fr_300px] gap-8 md:gap-16 items-start relative z-10">
          <div className="flex flex-col w-full">
            <div className="space-y-4 mb-6">
              <div className="flex flex-wrap items-center gap-3 mb-2">
                <div className="px-3 py-1 bg-sky-500 text-black text-[10px] md:text-xs font-bold shadow-[0_0_10px_rgba(14,165,233,0.5)] uppercase">
                  {HOME_DATA.userId}
                </div>
                <div className="px-3 py-1 border border-emerald-500/40 text-emerald-500 text-[9px] md:text-[10px] font-bold tracking-[0.2em] uppercase">
                  STATUS: WORKING
                </div>
              </div>
              
              <h2 className="text-3xl md:text-6xl font-bold glow-text leading-tight uppercase">
                {HOME_DATA.heroTitle[0]} <br className="hidden md:block" /> <span className="text-sky-400">{HOME_DATA.heroTitle[1]}</span>
              </h2>
          ...
          <div className="flex flex-col items-center gap-4 w-full md:w-auto md:shrink-0 order-first md:order-last">
            <div 
              className="w-48 h-48 md:w-64 md:h-64 border border-sky-500 p-2 relative group cursor-pointer"
            ...
            <div className="text-center">
              <p className="text-base md:text-xl font-bold text-sky-400 tracking-[0.3em] glow-text uppercase">{HOME_DATA.userName}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <section className="bg-black/80 border border-white/10 p-5 md:p-6 rounded flex flex-col h-[400px] md:h-[450px] shadow-lg">
          <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6 border-b border-white/5 pb-4">
            ...
            <button 
              ...
              className="w-full lg:w-auto px-5 py-2 border border-emerald-500 text-emerald-500 text-[9px] md:text-[10px] font-bold hover:bg-emerald-500 hover:text-black transition-all active:scale-95 shadow-[0_0_15px_rgba(16,185,129,0.1)] uppercase tracking-widest"
            >
              [RUN_DIAGNOSTICS]
            </button>
          </header>
          <div ref={logContainerRef} className="flex-1 space-y-1.5 overflow-y-auto scroll-smooth pr-2 custom-scrollbar font-mono">
            ...
            {activeLogs.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center space-y-4 opacity-20 text-center px-4">
                ...
                <div className="w-full max-w-[200px] h-px bg-gradient-to-r from-transparent via-emerald-500 to-transparent"></div>
              </div>
            )}
          </div>
        </section>
